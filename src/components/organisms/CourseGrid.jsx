...
const loadCourses = async () => {
    try {
      ...
      // Use Apper-SDK courseService to fetch courses
      const fetchedCourses = await courseService.getAll(searchQuery, selectedCategory, priceRange);
...
const filteredAndSortedCourses = React.useMemo(() => {
    ...
    // Filter by search query (already handled by API, but keep for client-side filtering)
    if (searchQuery.trim()) {
    ...
    // Filter by category (already handled by API, but keep for client-side filtering)
    if (selectedCategory !== "전체") {
      filtered = filtered.filter(course => course.category === selectedCategory);
    }

    // Filter by price range (already handled by API, but keep for client-side filtering)
    if (priceRange.min !== "") {
